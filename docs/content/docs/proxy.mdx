...
| `config` | object | no | Compression options, below. Unknown keys and invalid values return `400 invalid_request`. |
...
| `compress_system_messages` | boolean | unset | Allow compression of system-role messages. |
| `target_ratio` | number | unset | Target compression ratio in `(0, 1]`. |
| `protect_recent` | integer | unset | Leave the last N messages uncompressed. Must be non-negative. |
| `protect_analysis_context` | boolean | unset | Preserve analysis context blocks. |
| `min_tokens_to_compress` | integer | unset | Skip compression below this token count. Must be non-negative. |

Only keys the caller sends override server-level configuration, including
`HEADROOM_SAVINGS_PROFILE`. `kompress_model` and `savings_profile` remain
operator-level: remote callers cannot trigger arbitrary model downloads or
change the server's overall savings posture per request.
