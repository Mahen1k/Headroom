...
def test_defers_non_core_and_injects_search_tool(monkeypatch):
    ...
    for core_name in _CORE:
        assert not by_name[core_name].get("defer_loading")  # core stays resident
    for noncore_name in _NONCORE:
        assert by_name[noncore_name].get("defer_loading") is True  # non-core deferred
...
def test_supported_clients_keep_the_opted_in_deferral(monkeypatch):
    # The exclusion is per-client: after the operator opts in, clients that can
    # execute the injected search tool still receive the deferred payload.
    monkeypatch.setenv("HEADROOM_OPENAI_TOOL_SEARCH", "1")
    tools = _tools()
